...
#returns max GB of memory used in this job
def max_rss_gib():
    # Linux reports ru_maxrss in KiB
    return resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024**2

#returns current GB of memory used in this job
def current_rss_gib():
		...
		if make_data or self.overwrite_data:
			...
			if os.path.isfile(oct_file) and not self.overwrite_data:
				...
				if not make_pcd_data and not self.overwrite_data:
					...
					print(
					    f"Generated {len(point_cloud):,} points "
					    f"({point_cloud.nbytes / 1024**3:.2f} GiB)",
					    flush=True,
					)
					...
					print(
					    f"Current RSS after constructing point cloud: "
				...
				print("pre octree call", flush=True)
				...
				# print(f"Point memory: {points.nbytes / 1024**3:.3f} GiB", flush=True)
				# print(f"All finite: {np.isfinite(points).all()}", flush=True)
				# print(f"Coordinate minimum: {points.min(axis=0)}", flush=True)
				# print(f"Coordinate maximum: {points.max(axis=0)}", flush=True)
				# print(f"Coordinate span: {np.ptp(points, axis=0)}", flush=True)
				# print(f"Maximum magnitude: {np.abs(points).max():.6e}", flush=True)
				# print(f"Octree max depth: {octree.max_depth}", flush=True)
				# print(f"RSS before octree: {max_rss_gib():.2f} GiB", flush=True)
