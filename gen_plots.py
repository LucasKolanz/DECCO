...
if __name__ == '__main__':
	...
	relax = False
	if dataset_name.split('_')[-1] == "relax": #TODO need new way to indicate relax jobs
		relax = True
	...
	# porositiesabcavg = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
	# porositiesabcstd = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
	# yerr_abc = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
	# ABC_numruns = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
	
	# porositiesKBMavg = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
	# porositiesKBMstd = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
	# yerr_KBM = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
	# KBM_numruns = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
	
	# contactsavg = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
	# contactsstd = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
	# yerr_ca = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
	# contacts_numruns = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
	
	FD_dataavg = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
	FD_datastd = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
	yerr_FD = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
	FD_numruns = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
	
	# angmomavg = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
	# angmomstd = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
	# yerr_angmom = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
	# angmom_numruns = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)

	# BD_dataavg = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
	# BD_datastd = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
	# yerr_BD = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
	# BD_numruns = np.full(shape=data_shape,fill_value=np.nan,dtype=np.float64)
