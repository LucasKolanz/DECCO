...
def find_connected_particle_groups(pos, radii, timestep=-1, tol=1e-12):
    """
    Find connected groups of particles at one timestep.

    Parameters
    ----------
    pos : ndarray
        Particle positions with shape (timesteps, balls, 3)
    radii : ndarray
        Particle radii with shape (balls,) or (timesteps, balls)
    timestep : int
        Which timestep to analyze. Default is -1, the final timestep.
    tol : float
        Small tolerance added to contact criterion.

    Returns
    -------
    groups : list[list[int]]
        Connected particle index groups.
    """

    p = np.asarray(pos[timestep])  # shape (balls, 3)

    r = np.asarray(radii)
    if r.ndim == 2:
        r = r[timestep]           # shape (balls,)
...
def kabsch_align_segment_to_previous(pos, prev_pos, masses=None, common_indices=None):
    """
    Rigidly rotate + translate this segment so pos[0] matches prev_pos[-1]
    as well as possible for common particles.

    Applies the same rigid transform to every timestep and every particle
    in the new segment.
    """
    ...
    if common_indices is None:
        common = min(prev_pos.shape[1], pos.shape[1])
        common_indices = np.arange(common)

    A = pos[0, common_indices, :]       # current segment first frame
    B = prev_pos[-1, common_indices, :] # previous segment last frame

    if masses is None:
        w = np.ones(len(common_indices))
    else:
        w = np.asarray(masses)[common_indices]

    w = w / np.sum(w)

    A_com = np.sum(A * w[:, None], axis=0)
    B_com = np.sum(B * w[:, None], axis=0)
    ...
    # Apply to all particles in all timesteps
    pos = (pos - A_com[None, None, :]) @ R.T + B_com[None, None, :]
...
def center_on_target(pos, m, r):
    pos = np.asarray(pos).copy()
    m = np.asarray(m)
    r = np.asarray(r)

    groups = find_connected_particle_groups(pos, r, timestep=0, tol=1e-8 * np.min(r))
    groups = sorted(groups, key=len, reverse=True)

    target_group = groups[0]
    target_m = m[target_group]

    target_pos = pos[:, target_group, :]  # shape: timesteps, target_balls, xyz

    target_COM = np.sum(target_pos * target_m[None, :, None], axis=1) / np.sum(target_m)
    # shape: timesteps, xyz
...
#        print(simData.shape)        
        ...
        if prev_pos is not None:
            pos = kabsch_align_segment_to_previous(pos, prev_pos, masses=m)
#        simData = simData[i%10]
        prev_pos = pos.copy()
        print(f"Steps: {steps}")
#	    print(simData.shape)
