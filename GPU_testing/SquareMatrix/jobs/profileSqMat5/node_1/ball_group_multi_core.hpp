...
void Ball_group::sim_looper()
{
    ...
    #pragma acc enter data copyin(this,test) 
    ...
    #pragma acc exit data delete(dt,num_pairs,num_particles,Ha,k_in,k_out,h_min,u_s,u_r,\
        world_rank,world_size)
    #pragma acc exit data delete(this,test)
...
void Ball_group::sim_one_step(const bool writeStep)
{
    ...
    std::cerr<<"NUM PAIRS: "<<num_pairs<<std::endl;
    std::cerr<<"world_rank: "<<world_rank<<std::endl;
    std::cerr<<"world_size: "<<world_size<<std::endl;

    #pragma acc parallel loop gang worker reduction(+:pe) \
        present(test,pe,this,accsq[0:num_particles*num_particles],\
            aaccsq[0:num_particles*num_particles],m[0:num_particles],\
            moi[0:num_particles],w[0:num_particles],vel[0:num_particles],\
            pos[0:num_particles],R[0:num_particles],distances[0:num_pairs],\
            num_pairs,num_particles,Ha,k_in,k_out,h_min,u_s,u_r,writeStep,\
            world_rank,world_size)
    ...
    #pragma acc update host(accsq[:num_particles*num_particles],test)
    // for (int i = 0; i < 100; i++)
    // {
    //     std::cerr<<"accsq: "<<accsq[i].x<<','<<accsq[i].y<<','<<accsq[i].z<<std::endl;
    // }
    std::cerr<<"TEST AFTER: "<<test.x<<','<<test.y<<','<<test.z<<std::endl;
    std::cerr<<"acc[0] before update: "<<acc[0].x<<','<<acc[0].y<<','<<acc[0].z<<std::endl;
    ...
    #pragma acc update host(acc[0])
    std::cerr<<"acc[0] after update: "<<acc[0].x<<','<<acc[0].y<<','<<acc[0].z<<std::endl;
    ...
    std::cerr<<"vel[0] before update: "<<vel[0].x<<','<<vel[0].y<<','<<vel[0].z<<std::endl;
    ...
    #pragma acc update host(vel[0])
    std::cerr<<"vel[0] after update: "<<vel[0].x<<','<<vel[0].y<<','<<vel[0].z<<std::endl;
