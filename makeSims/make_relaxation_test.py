...
if __name__ == '__main__':
	...
	job_set_name_copy_from = "lognorm"
	job_group_copy_from = "jobsCosine"

	job_set_name = "relaxTest"
	job_group = "jobs"
	...
	for attempt in attempts:
		for n in N:
			for Temp in Temps:
				...
				copyjob = default_input_json["data_directory"] + f'{job_group_copy_from}/' + job_set_name_copy_from + str(attempt) + '/'\
							+ 'N_' + str(n) + '/' + 'T_' + str(Temp) + '/'
				...
				if os.path.exists(copyjob+"timing.txt"):
					...
					if not os.path.exists(job+"timing.txt"):
						...
						# input_json['u_s'] = 0.5
						# input_json['u_r'] = 0.5
						input_json['note'] = "Relaxation test"
